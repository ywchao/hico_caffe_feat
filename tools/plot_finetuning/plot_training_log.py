...
def get_data_file(chart_type, output_dir, path_to_log):
    return output_dir + '/' + os.path.basename(path_to_log) + '.' + get_data_file_type(chart_type).lower()
    ...
    with open(data_file, 'r') as f:
        for line in f:
            ...
            if line[0] != '#':
                ...
                # [ywchao]
                if field_idx0 > len(fields)-1 or field_idx1 > len(fields)-1:
                    break
...
def plot_chart(chart_type, output_path, path_to_log_list, display):

    path_to_png = output_dir + str(chart_type) + '.png'
    
    for path_to_log in path_to_log_list:
        os.system('%s %s %s' % (get_log_parsing_script(), path_to_log, output_dir))
        data_file = get_data_file(chart_type, output_dir, path_to_log)   
...
def print_help():
    print ''
    print """Usage:
    python plot_training_log.py chart_type[0-%s] net_type /path/to/first.log ...""" % (len(get_supported_chart_types()) - 1)
    print """Notes:
    1. Supporting multiple logs.
    2. Log file name must end with the lower-cased "%s".""" % get_log_file_suffix()
    print """Supported chart types:"""
    print """   -1: run all"""
    print """   -2: run even chart types"""
    supported_chart_types = get_supported_chart_types()
...
if __name__ == '__main__':
    if len(sys.argv) < 4:
        ...
        if chart_type >= 0:
            plot_chart(chart_type, output_dir, path_to_logs, True)

        if chart_type == -1:
            for c in range(len(get_supported_chart_types())):
                plot_chart(c, output_dir, path_to_logs, False)
